...
import { StoryRecallContent, ExerciseStoryContext } from '@/lib/db/types';

interface Props {
    question: {
        content: StoryRecallContent;
    };
...
export default function StoryRecallQuestion({ question, storyContext, onAnswer, disabled }: Props) {
    ...
    if (options.length === 0) {
        return (
            <div className="h-full flex flex-col items-center justify-center p-6">
                <p className="text-neutral-400">Question data is incomplete.</p>
            </div>
        );
    }
        ...
        <div className="h-full flex flex-col py-8 font-sans">
            ...
            <div className="mb-10 text-center">
                <h1 className="text-3xl md:text-4xl font-serif text-neutral-900 leading-tight mb-2">
                    {content?.question || 'What happened in the story?'}
                </h1>
                <p className="text-[11px] uppercase tracking-[0.2em] text-neutral-400">
                    Recall from the context
                </p>
            </div>

            {/* Story Context Card */}
            {content?.relatedParagraph && (
                <motion.div
                ...
                >
                    ...
                    <p className="text-base text-neutral-800 leading-relaxed font-serif">
                        &ldquo;{content.relatedParagraph}&rdquo;
                    </p>
                </motion.div>
            )}
