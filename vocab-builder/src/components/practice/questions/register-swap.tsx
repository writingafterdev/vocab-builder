...
import { RegisterSwapContent, ExerciseStoryContext } from '@/lib/db/types';

interface Props {
    question: {
        content: RegisterSwapContent;
    };
...
export default function RegisterSwapQuestion({ question, storyContext, onAnswer, disabled }: Props) {
    const content = question.content;
    const [selectedIndex, setSelectedIndex] = useState<number | null>(null);
    const [startTime] = useState(Date.now());

    const options = (content as any).options || (content as any).choices || (content as any).answers || [];
    const correctIndex = (content as any).correctIndex ?? (content as any).correctAnswer ?? 0;
    const originalText = (content as any).originalText || (content as any).text || '';
    const fromRegister = (content as any).currentRegister || (content as any).startRegister || 'informal';
    const toRegister = (content as any).targetRegister || (content as any).endRegister || 'formal';
        ...
        <div className="h-full flex flex-col py-8 font-sans">
            ...
            <div className="mb-10 text-center">
                ...
                <p className="text-[11px] uppercase tracking-[0.2em] text-neutral-400">
                    Transform the register
                </p>
            </div>

            {/* Original + Register Swap Card */}
            <motion.div
            ...
            >
                ...
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <span className="text-[10px] uppercase tracking-[0.2em] text-neutral-900 font-semibold">
                        {toRegister}
                    </span>
                </div>

                <p className="text-base font-serif text-neutral-800 leading-relaxed italic">
